...
\begin{frame}{Obiettivi della tesi}
    \begin{itemize}
        ...
        \textbf{Articolo}: \emph{Resolution Calculi for Non-normal Modal Logics}, Tableaux 2023.
        
        \textbf{autori}: Dirk Pattinson, Nicola Olivetti e Clàudia Nalon.
        \vspace{10px}
    \end{itemize}
...
\begin{frame}{Il metodo di risoluzione}
    \begin{itemize}
        \item \textbf{metodo di risoluzione}: calcolo per inferire la soddisfacibilità di un \emph{insieme di clausole} $S$.
        \vspace{10px}
        \item \textbf{Clausola}: disgiunzione di letterali, rappresentabile come insieme di letterali.
        \vspace{10px}
        \item Un insieme di clausole è la rappresentazione insiemistica della \emph{Forma normale congiuntiva} (CNF) di una formula.
    \end{itemize}
    \vspace{20px}
...
\begin{frame}{Il metodo di risoluzione per la logica classica}
    \begin{itemize}
        \item Le clausole contengono solo letterali proposizionali.
        \vspace{10px}
...
\begin{frame}{Regola di risoluzione}
    \begin{itemize}
        ...
        Esempio:
        \[
            \AXC{$\overbrace{\{ a, \textcolor{red}{b} \}}^{C_1}$}
            \AXC{$\overbrace{\{\textcolor{red}{\lnot b}, c, d \}}^{C_2}$}
            \RightLabel{Res}
            \BIC{$\underbrace{\{ a, c, d \}}_{R}$}
            \DP
        \]
        \item La risolvente $R$ viene aggiunta a $S$ se non è tautologica e non è già presente in $S$.
    \end{itemize}
...
\begin{frame}{Clausificazione di una formula}
    \begin{itemize}
        ...
        \item \textcolor{red}{\textbf{Clausificazione}}: conversione di una formula in insieme di clausole equivalente.
        \item per effettuare la clausificazione si applicano le regole di riscrittura:
        \begin{itemize}
        ...
        \item Esempio: $\quad F = (a \lor b) \to c$
        
        In CNF: $\quad (\lnot a \lor c) \land (\lnot b \lor c)$
        
        Insieme di clausole: $S = \{ \; \{\lnot a, c\}, \{\lnot b, c\} \; \}$
    \end{itemize}
...
\begin{frame}{Metodo di risoluzione per le logiche modali non-normali}
    \begin{itemize}
        \item I letterali possono essere \emph{proposizionali} o \textcolor{red}{\emph{modali}}. 
        ...
        \end{itemize}
        dove $l_i$ è un letterale (proposizionale o modale).
...
\begin{frame}{Regole di risoluzione $RES_E$}
    \begin{itemize}
        ...
        \end{figure} 
        \item $C$ e $D$ sono clausole, $l$ è un letterale, e $p$ è una variabile proposizionale.
...
\begin{frame}{Verificare la validità di una formula}
    \begin{itemize}
        \item L'utilizzo pratico implementato è la verifica della \textbf{validità} di una formula $F$ in input.
        \vspace{5px}
        \begin{center}
